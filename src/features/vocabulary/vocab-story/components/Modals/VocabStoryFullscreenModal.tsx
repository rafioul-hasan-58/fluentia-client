...
export const VocabStoryFullscreenModal: React.FC<VocabStoryFullscreenModalProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] w-screen h-screen bg-slate-100/95 dark:bg-[#0c0a17]/95 backdrop-blur-md text-slate-900 dark:text-white flex flex-col overflow-hidden animate-in fade-in duration-200">
      ...
      <div className="shrink-0 w-full px-3 sm:px-8 py-2.5 sm:py-3 bg-white/95 dark:bg-[#131024]/95 border-b border-amber-500/20 dark:border-amber-500/20 flex items-center justify-between gap-2 sm:gap-4 z-30 shadow-xs flex-nowrap backdrop-blur-md">
        {/* inset-inline-start: Story Counter */}
        <div className="flex items-center gap-2 text-xs sm:text-sm shrink-0 whitespace-nowrap">
          <div className="px-3 py-1.5 rounded-xl bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/25 text-amber-800 dark:text-amber-200 flex items-center gap-1.5 font-semibold">
            <span>Story</span>
            <span className="font-bold text-amber-600 dark:text-amber-400">#{activeStoryIndex + 1}</span>
            <span className="text-amber-600/70 dark:text-amber-400/70 text-[11px]">of {totalStories}</span>
          </div>
        </div>

        {/* Center: Story Navigation Controls */}
        <div className="flex items-center flex-nowrap shrink-0 gap-1 sm:gap-1.5 bg-amber-500/10 dark:bg-amber-500/15 p-1 rounded-xl border border-amber-500/30 dark:border-amber-500/30 whitespace-nowrap select-none">
          <button
            onClick={() => onNavigateStory(-1)}
            title="Previous Story (← Arrow key)"
            className="inline-flex items-center justify-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg hover:bg-amber-500/20 dark:hover:bg-amber-500/25 active:bg-amber-500/30 text-amber-800 dark:text-amber-200 text-xs font-semibold transition-all cursor-pointer shadow-2xs shrink-0 whitespace-nowrap"
          >
            <ChevronLeft className="w-3.5 h-3.5 shrink-0" />
            <span className="hidden sm:inline">Prev</span>
          </button>

          <span className="text-xs font-bold text-amber-900 dark:text-amber-200 px-1.5 sm:px-2 font-mono whitespace-nowrap shrink-0 select-none min-w-[3rem] text-center">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-1.5 sm:p-2 rounded-xl bg-slate-100 hover:bg-amber-500/15 dark:bg-slate-800 dark:hover:bg-amber-500/20 text-slate-600 hover:text-amber-800 dark:text-slate-300 dark:hover:text-amber-200 transition-colors cursor-pointer border border-slate-200 dark:border-slate-700 hover:border-amber-500/30 shrink-0"
