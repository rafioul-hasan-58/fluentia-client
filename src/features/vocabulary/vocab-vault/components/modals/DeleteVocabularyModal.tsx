...
export default function DeleteVocabularyModal({
  ...
  }, [itemToDelete, isDeleting, setItemToDelete]);

  if (!isMounted || !itemToDelete) return null;

  const wordObj = itemToDelete.word;
  const wordText = wordObj?.word || (itemToDelete as any).wordText || "Word";
  const pos = wordObj?.partOfSpeech;
  const posConfig = pos ? POS_COLORS[pos] : null;
  ...
  return createPortal(
    ...
    >
      <div
        className="relative w-full max-w-md rounded-3xl bg-white dark:bg-[#141226] border border-rose-500/20 dark:border-rose-500/30 shadow-[0_25px_60px_-15px_rgba(244,63,94,0.3)] overflow-hidden animate-in zoom-in-95 duration-150 p-6 space-y-5"
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {wordObj?.banglaPronunciation && (
                <span className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-1.5 py-0.5 rounded border border-indigo-200/60 dark:border-indigo-800/60">
                  উচ্চারণ: {wordObj.banglaPronunciation}
                </span>
              )}
            </div>
            {wordObj?.banglaMeaning && (
          ...
          {pos && (
            <span
              className={`px-2 py-0.5 rounded-md text-[11px] font-semibold border ${
                posConfig?.bg || "bg-indigo-500/10"
              } ${posConfig?.text || "text-indigo-600 dark:text-indigo-400"} ${
                posConfig?.border || "border-indigo-500/30"
              }`}
            >
              {posConfig?.label || pos}
            </span>
