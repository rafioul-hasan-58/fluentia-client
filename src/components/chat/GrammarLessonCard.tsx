...
export function GrammarLessonCard({ data, onActionSelect }: GrammarLessonCardProps) {
  ...
  return (
    <div className="w-full bg-paper border border-slate-200 dark:border-white/10 rounded-2xl p-3.5 sm:p-5 md:p-6 shadow-sm space-y-4 sm:space-y-5 text-ink animate-fadeIn">
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-white/10 pb-3.5 sm:pb-4">
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          <div>
            ...
            <h3 className="text-base sm:text-lg md:text-xl font-bold text-ink mt-0.5 font-display">
              {lesson.title}
            </h3>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-center">
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold transition-all border ${
              isSpeakingRule
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold bg-paper-card border border-slate-200 dark:border-white/10 hover:border-primary/40 text-ink-soft hover:text-ink transition-all"
      ...
      <div className="bg-primary/5 dark:bg-blue-950/40 border border-primary/20 dark:border-blue-500/30 rounded-xl p-3 sm:p-4 space-y-1.5 sm:space-y-2">
        ...
        <p className="text-xs sm:text-sm md:text-[15px] font-medium text-ink leading-relaxed">
          {lesson.rule}
        </p>
      </div>
      ...
      <div className="space-y-2.5 sm:space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="text-[11px] sm:text-xs font-bold text-ink-soft uppercase tracking-wider flex items-center gap-1.5">
            <span>✨</span>
            <span>Real-World Examples ({lesson.examples.length})</span>
          </h4>
          <span className="text-[10px] sm:text-[11px] text-ink-soft/70">Tap 🔊 to hear voice</span>
        </div>
      ...
      {lesson.commonMistakes && lesson.commonMistakes.length > 0 && (
        <div className="bg-rose-500/5 dark:bg-rose-950/20 border border-rose-500/20 dark:border-rose-500/30 rounded-xl p-3 sm:p-4 space-y-2">
          <div className="flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider">
            <span>⚠️</span>
            <span>Common Pitfalls to Avoid</span>
          </div>

          <ul className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-ink-soft dark:text-slate-300">
      ...
      {onActionSelect && (
        <div className="pt-2 border-t border-slate-200 dark:border-white/10 space-y-2">
          <span className="text-[10px] sm:text-[11px] font-bold text-ink-soft uppercase tracking-wider block">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            <button
              ...
              className="px-2.5 sm:px-3 py-1.5 rounded-lg bg-paper-card border border-slate-200 dark:border-white/10 hover:border-primary hover:bg-primary/5 text-[11px] sm:text-xs font-semibold text-ink hover:text-primary transition-all flex items-center gap-1.5 shadow-2xs"
            >
              <span>🎯</span>
              <span>Quiz Me</span>
            </button>
      ...
      {sessionId && (
        <div className="flex items-center justify-between text-[9px] sm:text-[10px] text-ink-soft/60 pt-1">
