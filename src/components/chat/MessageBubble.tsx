    ...
    <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} space-y-2 group`}>
      ...
      <div
        className={`max-w-[88%] md:max-w-[78%] rounded-2xl p-4 text-sm leading-relaxed transition-all shadow-sm ${
          isUser
            ? "bg-primary text-white rounded-tr-none shadow-sm dark:shadow-[0_0_20px_rgba(124,58,237,0.3)]"
        ...
        {!isUser && (
          <div className="mt-3 pt-2.5 border-t border-slate-200 dark:border-white/10 flex items-center justify-between">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs text-primary dark:text-purple-300 hover:text-primary-dark font-medium transition-colors"
            >
      ...
      {!isUser && message.feedback && showFeedback && (
        <div className="max-w-[88%] md:max-w-[78%] bg-primary/10 dark:bg-purple-950/40 border border-primary/20 dark:border-purple-500/30 rounded-xl p-3.5 space-y-2.5 text-xs">
          <div className="flex items-center justify-between font-semibold text-primary dark:text-purple-300">
            <div className="flex items-center gap-1.5">
              <span>💡</span>
              <span>Language Coaching Notes</span>
            </div>
            {message.feedback.vocabularyScore && (
              <span className="bg-primary/20 text-primary dark:text-purple-300 border border-primary/30 px-2 py-0.5 rounded-full text-[10px] font-bold">
          ...
          {message.feedback.betterAlternatives && (
            <div className="space-y-1 pt-1.5 border-t border-primary/15 dark:border-purple-500/20">
              ...
              <ul className="list-disc list-inside space-y-0.5 text-ink-soft dark:text-slate-300 pl-1">
                {message.feedback.betterAlternatives.map((alt, idx) => (
                  <li key={idx} className="hover:text-primary dark:hover:text-purple-300 transition-colors cursor-pointer">
