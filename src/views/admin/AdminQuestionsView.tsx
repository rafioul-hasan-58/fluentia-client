...
import {
  ...
  addQuestionsToSetApi,
  removeQuestionsFromSetApi,
...
export function AdminQuestionsView() {
  ...
  const handleOpenEditSetModal = async (set: LevelTestQuestionSet, e?: React.MouseEvent) => {
    ...
    try {
      ...
      if (fullSetRes.success && fullSetRes.data?.questions) {
        const ids = fullSetRes.data.questions.map((q) => q.id);
  ...
  const handleRemoveQuestionFromInspectSet = async (questionId: string) => {
    if (!inspectingSet) return;
    if (!window.confirm("Are you sure you want to remove this question from this set?")) return;

    setRemovingQuestionId(questionId);
    try {
      const res = await removeQuestionsFromSetApi(inspectingSet.id, [questionId]);
      if (res.success) {
        triggerAlert("Question removed from set.", "success");
        // Refetch set details
        const refreshed = await fetchAdminQuestionSetById(inspectingSet.id);
        if (refreshed.success && refreshed.data) {
          setInspectingSet(refreshed.data);
        } else {
          setInspectingSet((prev) =>
            prev
              ? {
                  ...prev,
                  questions: prev.questions?.filter((q) => q.id !== questionId),
                  questionsCount: Math.max(0, (prev.questionsCount || 1) - 1),
                }
              : null
          );
        }
        ...
        triggerAlert(res.message || "Failed to remove question from set.", "warn");
      }
    } catch (err: any) {
      triggerAlert(err.message || "Error removing question from set.", "warn");
    } finally {
  ...
  const handleSubmitSet = async (e: React.FormEvent) => {
    ...
    if (!editingSet && selectedQuestionIds.length === 0) {
    ...
    try {
      if (editingSet) {
        // 1. Update basic set properties (name, description, isActive)
        ...
        const updateRes = await updateAdminQuestionSetApi(editingSet.id, updatePayload);

        // 2. Identify newly added questions
        const addedIds = selectedQuestionIds.filter((id) => !initialQuestionIds.includes(id));
        if (addedIds.length > 0) {
          await addQuestionsToSetApi(editingSet.id, addedIds);
        }

        // 3. Identify removed (deselected) questions
        const removedIds = initialQuestionIds.filter((id) => !selectedQuestionIds.includes(id));
        if (removedIds.length > 0) {
          await removeQuestionsFromSetApi(editingSet.id, removedIds);
        }

        if (updateRes.success) {
          triggerAlert("Question set updated successfully!", "success");
          setIsSetModalOpen(false);
          resetSetForm();
          await loadQuestionSets();
          if (inspectingSet?.id === editingSet.id) {
            const refreshed = await fetchAdminQuestionSetById(editingSet.id);
            if (refreshed.success && refreshed.data) {
              setInspectingSet(refreshed.data);
            }
          }
        } else {
          triggerAlert(updateRes.message || "Failed to update question set.", "warn");
        }
      } else {
        // CREATE SET: POST /level-test-questions/create-set
              ...
              {isLoadingSetDetails ? (
                ...
                <div className="space-y-2.5 max-h-[45vh] overflow-y-auto pr-1">
                  {inspectingSet.questions.map((q, idx) => (
                    <div
                      key={q.id || idx}
                      className="p-3.5 rounded-xl bg-paper border border-slate-200 dark:border-white/10 space-y-2 group/q"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2">
                        ...
                        {/* Quick Remove from Set Button */}
                        <button
                          type="button"
                          onClick={() => handleRemoveQuestionFromInspectSet(q.id)}
                          disabled={removingQuestionId === q.id}
                          className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold text-rose-500 hover:text-rose-600 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 transition-all opacity-80 group-hover/q:opacity-100 disabled:opacity-50"
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/10 pb-3">
              <div>
                ...
                <p className="text-xs text-ink-soft">
                  {editingSet
                    ? `Update question set configuration and select/deselect questions (ID: ${editingSet.id})`
